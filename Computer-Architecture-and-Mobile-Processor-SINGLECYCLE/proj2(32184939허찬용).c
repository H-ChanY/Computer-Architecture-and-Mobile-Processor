...
void Control(inst_ *ins, Control_* c) {
    if (ins->opcode == 0 && ins->func_!=jalr) c->RegDest = true;
    ...
    if (ins->opcode != sw && ins->opcode != bne && ins->opcode != beq && ins->opcode != j && ins->func_ != jr && ins->func_!=jalr) c->RegWrite = true;
...
void Memmory_and_store(inst_ *ins,Control_ *c) {
    ...
    }else if(c->RegDest && c->RegWrite &&!(c->MemWrite)&& !(c->MemtoReg)&& (ins->func_!=jr)&&!(ins->opcode==0&&ins->func_==jalr)){//R type
